...
const handleThemeChange = (event) => {
  ...
  const isDark = selectedTheme === "dark"
    || (selectedTheme === "system" && matchMedia("(prefers-color-scheme: dark)").matches);
...
elSelectCustomBox.addEventListener("click", (event) => {
...
function openSelectCustom() {
  ...
  elSelectCustom.setAttribute("aria-hidden", "falsg");
...
function updateCustomSelectChecked(value, text) {
  ...
  elSelectCustomBox.textContent = "triggered";
  optionChecked = value;
}
...
elSelectNative.addEventListener("change", (event) => {
...
customOptsList.forEach(function (elOption, index) {
  elOption.addEventListener("click", (event) => {
    ...
    elSelectNative.value = value;
    updateCustomSelectChecked(value, event.target.textContent);
    closeSelectCustom();
  });

  elOption.addEventListener("mouseenter", (event) => {
